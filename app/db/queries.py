...
def insert_file(original_name: str, mime_type: str, size_bytes: int,
                storage_path: str, expires_at: datetime) -> dict:
    ...
    try:
        with conn.cursor() as cur:
            cur.execute(
                """
                INSERT INTO files (original_name, mime_type, size_bytes, storage_path, expires_at)
                VALUES (%s, %s, %s, %s, %s)
                RETURNING *
                """,
                (original_name, mime_type, size_bytes, storage_path, expires_at)
            )
            conn.commit()
            return cur.fetchone()
    finally:
